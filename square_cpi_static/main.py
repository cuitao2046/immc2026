...
class WildlifeProtectionOptimizer:
    ...
    def _initialize_models(self, grid_type='hex'):
        """初始化模型，支持六边形或正方形网格
        
        Args:
            grid_type: 'hex' 或 'square'，指定网格类型
        """
        if grid_type == 'square':
            from square_grid_model import SquareGridModel
            self.grid_model = SquareGridModel(self.data_loader.grids)
            self.visualizer = Visualizer(self.grid_model, square_size=1.0)  # 使用square_size参数
        else:
            from grid_model import HexGridModel
    ...
    def setup_default_scenario(self, grid_width: int = 12, height: int = 10, grid_type='hex'):
        """设置默认场景，支持六边形或正方形网格"""
        print(f"设置默认场景: 矩形{grid_type}网格 {grid_width} x {height}")
        
        # 生成网格
        if grid_type == 'square':
            self.data_loader.generate_rectangular_square_grid(width=grid_width, height=height)
        else:
            self.data_loader.generate_rectangular_hex_grid(width=grid_width, height=height)
        ...
        if grid_type == 'square':
            from square_grid_model import SquareGridModel
            temp_grid_model = SquareGridModel(self.data_loader.grids)
        else:
            from grid_model import HexGridModel
        ...
        # 初始化模型
        self._initialize_models(grid_type=grid_type)
