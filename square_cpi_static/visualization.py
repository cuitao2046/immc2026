...
class Visualizer:
    def __init__(self, grid_model, square_size: float = 1.0):
        self.grid_model = grid_model
        self.square_size = square_size
        self.grid_ids = grid_model.get_all_grid_ids()

    def _get_grid_corners(self, grid_id: int) -> List[Tuple[float, float]]:
        return self.grid_model.get_grid_corners(grid_id, self.square_size)

    def _get_grid_center(self, grid_id: int) -> Tuple[float, float]:
        return self.grid_model.get_grid_center_coords(grid_id, self.square_size)
    ...
    def plot_risk_heatmap(self, save_path: str = None, show: bool = True):
        ...
        for grid_id in self.grid_ids:
            corners = self._get_grid_corners(grid_id)
        ...
        bounds = self.grid_model.get_grid_bounds(self.square_size)
    ...
    def plot_deployment_map(self, solution: DeploymentSolution, save_path: str = None, show: bool = True):
        ...
        for grid_id in solution.cameras.keys():
            center = self._get_grid_center(grid_id)
            circle = Circle(center, radius=self.square_size * 0.4, color='red', alpha=0.8, zorder=10)
            ax.add_patch(circle)
        ...
        for grid_id in solution.drones.keys():
            center = self._get_grid_center(grid_id)
            circle = Circle(center, radius=self.square_size * 0.4, color='blue', alpha=0.8, zorder=10)
            ax.add_patch(circle)
        ...
        for grid_id in solution.camps.keys():
            center = self._get_grid_center(grid_id)
            circle = Circle(center, radius=self.square_size * 0.5, color='green', alpha=0.8, zorder=10)
            ax.add_patch(circle)
        ...
        for edge_key in solution.fences.keys():
            ...
            center1 = self._get_grid_center(grid_id1)
            center2 = self._get_grid_center(grid_id2)
    ...
    def plot_protection_coverage(self, solution: DeploymentSolution, coverage_model, 
        ...
        # 为围栏保护创建一个空字典，因为CoverageModel没有calculate_fencing_edges方法
        fence_prot = {grid_id: 0.0 for grid_id in self.grid_ids}
    ...
    def plot_protection_heatmap(self, solution: DeploymentSolution, coverage_model, 
                                save_path: str = None, show: bool = True):
        """绘制保护程度热力图"""
        fig, ax = plt.subplots(figsize=(12, 10))

        # 计算每个网格的保护效益
        protection_benefit = coverage_model.calculate_protection_benefit(solution)

        max_benefit = max(protection_benefit.values()) if protection_benefit else 1.0
        min_benefit = min(protection_benefit.values()) if protection_benefit else 0.0
        ...
        for grid_id in self.grid_ids:
            ...
            normalized_benefit = (benefit - min_benefit) / (max_benefit - min_benefit) if max_benefit > min_benefit else 0.5
            color = cmap(normalized_benefit)

            polygon = Polygon(corners, facecolor=color, edgecolor='black', linewidth=0.5, alpha=0.8)
            ax.add_patch(polygon)

        sm = plt.cm.ScalarMappable(cmap=cmap, norm=plt.Normalize(vmin=min_benefit, vmax=max_benefit))
        sm.set_array([])
        cbar = plt.colorbar(sm, ax=ax, shrink=0.8)
        ...
        if save_path:
            plt.savefig(save_path, dpi=300, bbox_inches='tight')
            print(f"Protection heatmap saved to {save_path}")
